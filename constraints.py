...
class QueensTableConstraint(TableConstraint):
	...
	for k in range(len(i_dom)):
	    j_dom = qj.curDomain()
	
	    diag_minus = i_dom[k] - abs(i-j)
	    diag_plus = i_dom[k] + abs(i-j)
	 
 	    if i_dom[k] in j_dom:
		j_dom.remove(i_dom[k])
	    if diag_minus in j_dom:
		j_dom.remove(diag_minus)
	    if diag_plus in j_dom:
		j_dom.remove(diag_plus)

	    for l in range(len(j_dom)):
		list.append([i_dom[k],j_dom[l]])

	TableConstraint.__init__(self,self._name,scope,list)
...
class NValuesConstraint(Constraint):
	...
	for v in self.scope():
		...
		return True
	return (count >= self._lb and count <= self._up)
	...
	def count_in_range(l):
	    vals = [val for (var,val) in l]
	    vars = [var for (var,val) in l]
		...
		if v not in vars:
		    v_vals = v.domain()
		    if self._required in v_vals:
			unassigned_count +=1

	    if count + unassigned_count < self._lb:
		return False
        ...
        varsToAssign = self.scope()
        varsToAssign.remove(var)
        x = findvals(varsToAssign, [(var, val)], count_in_range, count_in_range)
	return x
